...
#define REMOTEXY_WIFI_SSID "OTTO GLOW"
...
uint8_t RemoteXY_CONF[] = {
    255, 6,   0,  0,  0,  66, 0,  13, 8,   0,   5,   32,  3,   12,  41,
    41,  1,   26, 31, 1,  3,  79, 16, 16,  12,  1,   31,  82,  240, 159,
    166, 190, 0,  1,  3,  56, 39, 18, 12,  1,   31,  240, 159, 146, 191,
    0,   1,   3,  79, 39, 17, 12, 1,  31,  240, 159, 166, 191, 0,   1,
    3,   56,  16, 17, 12, 1,  31, 76, 240, 159, 166, 190, 0};
...
int LATL = 100;  // 0 = Full Tilt Right   180 = Full Tilt Left   Default BASIC =
                 // 85   Default HUMANOID = 80

// Right Leg tilt left walking position
int RATL = 175;  // 0 = Full Tilt Right   180 = Full Tilt Left   Default BASIC =
                 // 175   Default HUMANOID = 150

// Left Leg tilt right walking position
int LATR = 5;  // 0 = Full Tilt Right   180 = Full Tilt Left   Default BASIC = 5
               // Default HUMANOID = 30

// Right Leg tilt right walking position
int RATR = 80;  // 0 = Full Tilt Right   180 = Full Tilt Left   Default BASIC =
                // 95  Default HUMANOID = 100
...
const uint8_t ServoLeftFootPin = 33;   // D7
const uint8_t ServoLeftLegPin = 14;    // D8
const uint8_t ServoRightFootPin = 27;  // D3
const uint8_t ServoRightLegPin = 32;   // D4

const uint8_t ServoLeftArmPin = 2;   // D0
const uint8_t ServoRightArmPin = 5;  // RX
const uint8_t ServoHeadPin = 4;      // TX
...
void loop() {
  ...
  if (ModeCounter == 0) {
    ...
    if (RemoteXY.J_y > 0) {
      ...
      if ((millis() - currentmillis1 >= Interval1) &&
          (millis() - currentmillis1 <= Interval2)) {
        myservoRightFoot.write(90 - RFFWRS);
      }

      if ((millis() - currentmillis1 >= Interval2) &&
          (millis() - currentmillis1 <= Interval3)) {
        myservoRightFoot.detach();
        myservoLeftLeg.write(LATL);
        myservoRightLeg.write(RATL);
      }

      if ((millis() - currentmillis1 >= Interval3) &&
          (millis() - currentmillis1 <= Interval4)) {
        myservoLeftFoot.write(90 + LFFWRS);
      }

      if ((millis() - currentmillis1 >= Interval4) &&
          (millis() - currentmillis1 <= Interval5)) {
        myservoLeftFoot.detach();
      }
    }
